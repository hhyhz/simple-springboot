...
if aws ecs describe-services --service ${serviceName} --cluster ${clusterName} | grep "MISSING"
then
...
then
  ...
  aws ecs update-service --service ${serviceName} --task-definition ${taskFamily} --cluster ${clusterName}
fi
